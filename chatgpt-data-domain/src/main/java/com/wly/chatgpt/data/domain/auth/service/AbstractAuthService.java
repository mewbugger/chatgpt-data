...
public abstract class AbstractAuthService implements IAuthService{
    /**
     * SecretKey 密钥 其实最好通过配置的方式
     */
    private static final String defaultBase64EncodedSecretKey = "bcwly";
    ...
    public AuthStateEntity doLogin(String code) {
        ...
        String token = encode(authStateEntity.getOpenId(), 7 * 24 * 60 * 1000, chaim);
    ...
    protected String encode(String issuer, long ttlMillis, Map<String, Object> claims) {
        // iss签发人，ttlMillis生存时间，claims是指还想要jwt中存储的一些非隐私数据
        if (claims == null) {
            claims = new HashMap<>();
        }
        ...
        JwtBuilder builder = Jwts.builder()
                // 荷载部分
                .setClaims(claims)
                // 这是jwt的唯一标识，一般设置成唯一的，这个方法可以生成唯一标识
                .setId(UUID.randomUUID().toString())
                // 签发时间
                .setIssuedAt(new Date(nowMillis))
                // 签发人，也就是JWT是给谁的（逻辑上一般都是username或userId）
                .setSubject(issuer)
                //这个地方是生成jwt使用的算法和秘钥
                .signWith(SignatureAlgorithm.HS256, base64EncodedSecretKey);
        if (ttlMillis >= 0) {
            long expMillis = nowMillis + ttlMillis;
            // 过期时间，单位是毫秒，使用当前时间+前面传入的持续时间生成
